...
# january 30, 2018
#
# acs_county_plots_ch01_ch03_v04.R
#
##############################################################################
##############################################################################
...
p1 <- ggplot(df, aes(x = area_type, y = total_housing_units, fill =  group)) +
  ...
  theme(panel.grid.major = element_blank(),
        ...
        legend.spacing = unit(0.45,"cm"))+
           ...
           position = position_stack(vjust=0.5), #vjust =2,
           color = "white", size = 6)+
  geom_segment(aes(x = 1.35, y = 142326, xend = 1.55 , yend = 142326), arrow = arrow(length = unit(0.5, "cm")),color="#525252")+
  annotate(geom="text", x=1.12, y=142326, label="Fairfax & Falls Church cities", color="#525252", size=5)+
  ...
  annotate(geom="text", x=2.03, y=186363, label="Rappahannock, Clarke, Warren & Culpeper Counties", color="#525252", size=5)+
  geom_segment(aes(x = 2.25, y = 179215, xend = 2.55 , yend = 179215), arrow = arrow(length = unit(0.3, "cm")),color="#525252")+
